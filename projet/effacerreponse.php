<!-- Sur cette page, seul un utilisateur peut effacer sa réponse à une question d'un autre utilisateur -->
...
<html lang="fr">
    <head>
        <meta charset="UTF-8">
        <title>Effacer une de mes réponses</title>
        <link rel ="stylesheet" href ="effacerquestion.css">
    </head>
    <body>
        <?php if(!isset($_SESSION)){session_start();};
        if (isset($_SESSION['nomduti'])){
            if (!isset($_POST['reponses']) &&(!isset($_POST['motdepasse']))&&isset($_GET['post_id'])){
                echo "<form action=\"effacerreponse.php?post_id=".$_GET['post_id']."\" method=\"post\">";
                echo "Êtes-vous surs de vouloir effacer cette réponse ".$_SESSION['nomduti']."? <br><br>";
                ...
                echo "<input type=\"submit\" name=\"action\" class=\"buttons\">";
                echo "</form>";
            }else if ((isset($_POST['motdepasse']))){ //vérification du mot de passe pour sécurité
        ...
        function supprimer(){
            if (isset($_GET['post_id'])){ //le paramètre $_GET['post_id'] est donné en argument, j'ai trouvé cette façon la plus efficace pour transmettre l'id de la publication d'une page à une autre
                $connexion = mysqli_connect ('127.0.0.1','root','','projet');
                $req='DELETE FROM publication WHERE post_id = \''.$_GET['post_id'].'\'';
                $resultat = mysqli_query ($connexion,$req);
                echo "Réponse supprimée. <br>";
            }else{
