...
<?php session_start();
    if ((empty($_POST['nomduti']))||(empty($_POST['prenom']))||(empty($_POST['nom']))||(empty($_POST['date']))||(empty($_POST['email']))||(empty($_POST['mdp']))){
        ...
        if(mysqli_num_rows($select_1)) {
            echo '<script type="text/javascript">alert("Ce nom d\'utilisateur est déjà pris.")</script>';
            include_once('inscription.php');
        }else if(mysqli_num_rows($select_2)) {
            echo '<script type="text/javascript">alert("Un compte à déjà été créé avec cette adresse mail.")</script>';
            include_once('inscription.php');
        }else{
            $req='INSERT INTO users (prenom, nom, pseudo, mail, naissance, password) VALUES (\''.$prenom.'\',\''.$nom.'\',\''.$pseudo.'\',\''.$email.'\',\''.$date.'\',\''.$mdphash.'\');';
